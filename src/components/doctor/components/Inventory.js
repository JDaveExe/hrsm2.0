...
const Inventory = ({ currentPath, currentDateTime, simulationMode, isDarkMode = false, inventoryFilter = null }) => {
  ...
  const loadInventoryData = async () => {
    ...
    try {
      if (activeTab === 'vaccines') {
        // Load from the same data source as admin
        try {
          const response = await fetch('/backend/data/vaccines.json');
          if (response.ok) {
          ...
          console.error('Error fetching vaccines from file:', fetchError);
          // Fallback to service
          const vaccineData = await inventoryService.getAllVaccines();
          setVaccines(vaccineData || []);
        }
        ...
        // Load from the same data source as admin
        try {
          const response = await fetch('/backend/data/medications.json');
          if (response.ok) {
            const data = await response.json();
            setMedications(data || []);
          } else {
          ...
          console.error('Error fetching medications from file:', fetchError);
          // Fallback to service
          const medicationData = await inventoryService.getAllMedications();
          setMedications(medicationData || []);
        }
